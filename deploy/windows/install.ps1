...
# Пятнадцать, а не двадцать: замер показал, что одна сборка занимает
# около десяти (исходники, образ, кеш), а лишние пять — на то, что диск
# виртуалки WSL растёт и сам обратно не сжимается. Сама вики при этом
# весит десятки мегабайт: база 67 МБ за неделю работы.
...
else { Bad "на диске C свободно $freeGb ГБ, нужно хотя бы 10 ГБ" }
