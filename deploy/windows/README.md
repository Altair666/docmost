...
### Чтобы собрать образ на этой же машине

| | сколько |
|---|---|
| свободная память | **от 4 ГБ** |
| свободный диск | **от 10 ГБ** (Windows: 15, см. ниже) |
| время сборки | 10–20 минут |

Разница берётся не из воздуха: сборка добавляет исходники с зависимостями
(1.5 ГБ), промежуточные слои и кеш (3–5 ГБ за проход). На шести
гигабайтах памяти сборка клиента уже вешала машину целиком, когда рядом
работали другие контейнеры, — отсюда четыре гигабайта свободными.

На Windows просим пятнадцать вместо десяти: диск виртуалки WSL растёт по
мере работы и сам обратно **не сжимается**. Вернуть место можно только
вручную:
...
их при следующей сборке. Записи не удаляются сами: у нас за пятнадцать
проходов набежало 45 ГБ. `deploy/build.sh` теперь подрезает кеш старше
недели и оставляет три последних образа, но при частых сборках за местом
стоит поглядывать:
