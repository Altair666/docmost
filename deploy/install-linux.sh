...
# Сколько нужно на сборку. Цифры замерены, а не взяты на глаз.
#
# Память: на 6 ГБ сборка клиента уже вешала машину целиком, когда рядом
# работали другие контейнеры.
#
# Диск: сама вики невесома — база 67 МБ за неделю работы, вложения
# считанные килобайты. Место уходит на сборку: исходники с node_modules
# около 1.5 ГБ, готовый образ 1.8 ГБ, базовые образы 0.6 ГБ, кеш сборки
# 3–5 ГБ за проход. Отсюда десять: одна сборка помещается с запасом.
# Кеш при этом растёт от сборки к сборке — за пятнадцать проходов у нас
# набежало 45 ГБ, — поэтому deploy/build.sh теперь его подрезает.
MIN_FREE_MB=4000
MIN_DISK_GB=10
