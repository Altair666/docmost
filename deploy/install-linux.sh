...
Что дальше:
  1. Направьте на этот порт обратный прокси на адрес $APP_URL.
     Готовый конфиг nginx: $SRC_DIR/deploy/nginx-docmost.conf —
     в нём переброс веб-сокета и заголовки, без которых редактор
     уходит в режим чтения, а вход падает с 500.
