...
export default function GristAppShell({
  children,
}: {
  ...
  // Правая панель повторяет левую: свёрнута — полоса, черта у левого
  // края — растягивание. Выезда по наведению у неё нет.
    ...
    <>
      ...
      <AppShell
        ...
        navbar={{
          ...
          // у апстрима на главной и в настройках она жёстко 300px.
          //
        ...
        aside={
          isPageRoute && {
            // Панель не исчезает, а сжимается до полосы — как левая.
            width: rightOpen || isAsideOpen ? rightWidth : COMPACT_RAIL_WIDTH,
            breakpoint: "sm",
            collapsed: { mobile: !rightOpen, desktop: false },
          }
        }
        padding="md"
      >
        {/* Без горизонтального отступа: иначе левая ячейка шапки съезжает
            ...
            Отступы задают сами группы внутри шапки. */}
        ...
        >
          {/* Разметка одна на оба состояния, как у Grist: свёрнутый вид —
              ...
              Обёртка подрезает содержимое: пока панель едет, оно не должно
          ...
          >
            ...
            {/* Тянуть можно только развёрнутую панель. В свёрнутой полосы
                нет даже когда она временно выехала по наведению: менять
                там нечего, а край хватался и подсвечивался. */}
            {!collapsed && (
              <div
            ...
            {/* Ширина содержимого — целевая, а не всегда развёрнутая: иначе
                в свёрнутом виде пункты шире панели и лезут за край. Grist
                на время перехода ставит ровно её же. */}
            <div
              ref={contentRef}
              data-sidebar-content=""
              style={{
                width:
                  collapsed && !railHovered ? COMPACT_RAIL_WIDTH : sidebarWidth,
                flex: "1 1 auto",
                minHeight: 0,
              }}
            >
