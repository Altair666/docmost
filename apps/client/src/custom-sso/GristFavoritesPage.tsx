...
export default function GristFavoritesPage() {
  const { t } = useTranslation();
  // Без указания вида сервер отдаёт всё закреплённое — и страницы, и
  // пространства. Отдельной секции для закреплённых пространств в меню
  // больше нет, поэтому показываем их здесь же.
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage,
  } = useFavoritesQuery(undefined);
  const loaded = data?.pages.flatMap((p) => p.items) ?? [];

  // Порядок как у Grist: по имени, по дате (свежие сверху) или по автору.
  ...
  const shown = listFilter
    ? loaded.filter((f: any) => nameOf(f).toLowerCase().includes(listFilter))
    : loaded;
